...
def get_all_categories_with_top_videos(
    ...
    # Рекомендации для видео-1
    print("=== Рекомендации для 'Python Basics' ===")
    recs = get_category_recommendations("video-1", test_videos, limit=5)
    for v in recs:
        print(f"  - {v['name']} ({v['views']} views)")
    
    # Тренды
    print("\n=== Тренды ===")
    trending = get_trending_videos(test_videos, limit=3)
    for v in trending:
        print(f"  - {v['name']} ({v['views']} views)")
    
    # Видео по категориям
    print("\n=== По категориям ===")
    by_cat = get_all_categories_with_top_videos(test_videos, top_per_category=2)
    for cat in by_cat:
        print(f"  Category {cat['category_id']}:")
        for v in cat['top_videos']:
            print(f"    - {v['name']}")
